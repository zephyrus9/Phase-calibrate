...
%Usage:
%       [predicted_labels,accuracy] = KNN_(3,training,training_labels,testing,testing_labels)
...
for test_point=1:size(t_data,1)
    for train_point=1:size(data,1)
        %calc and stor sorted eculidean distances with corresponding indices
...
for i=1:size(k_nn,1)
    options=unique(k_nn(i,:));
    max_count=0;
    max_label=0;
    for j=1:length(options)
        if length(find(k_nn(i,:)==options(j)))>max_count
